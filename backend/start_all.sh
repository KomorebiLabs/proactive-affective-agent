#!/bin/bash
# ==============================================================================
# Wanqing AI - Startup Script (Linux/macOS)
# ==============================================================================
# Architecture: Redis(6379) + Perception(8000) + Agent(8001) + Java(8080) + Vue(5173)
# ==============================================================================
...
if [[ "$PYTHON_MAJOR" -lt 3 ]] || [[ "$PYTHON_MAJOR" -eq 3 && "$PYTHON_MINOR" -lt 10 ]]; then
    log_warn "Python 3.10+ recommended. Current version: $PYTHON_VERSION"
fi
...
if curl -s http://localhost:8001/health &> /dev/null; then
    ...
    # 安装依赖（如果没有虚拟环境，使用系统 Python）
    if [[ -f "$AGENT_DIR/requirements.txt" ]]; then
        log_info "Installing Agent Python dependencies..."
        pip3 install -q -r "$AGENT_DIR/requirements.txt" 2>/dev/null || \
            log_warn "Some dependencies failed to install. Check pip source and network."
    fi
    ...
    if ! curl -s http://localhost:8001/health &> /dev/null; then
        log_warn "Agent startup timeout (30s). Check logs: $AGENT_DIR/logs/agent.log"
    fi
fi
...
if curl -s http://localhost:8000/health &> /dev/null; then
    ...
    if [[ -f "$PERCEPTION_DIR/requirements.txt" ]]; then
        log_info "Installing Perception Python dependencies..."
    ...
    nohup $PERCEPTION_PYTHON "$PERCEPTION_DIR/main.py" \
        > "$PERCEPTION_DIR/logs/perception.log" 2>&1 &
    PERCEPTION_PID=$!
    ...
    if curl -s http://localhost:8000/health &> /dev/null; then
        ...
        log_warn "Perception service may not be ready. Check: $PERCEPTION_DIR/logs/perception.log"
    fi
fi
...
if curl -s http://localhost:8080/health &> /dev/null; then
    ...
    if command -v mvn &> /dev/null; then
        ...
        log_error "Maven (mvn) not found and mvnw missing. Please install Maven."
    ...
    nohup $MVN_CMD spring-boot:run \
        ...
        > "$BACKEND_DIR/logs/java.log" 2>&1 &
    JAVA_PID=$!
    log_info "Java process started (PID $JAVA_PID, takes 30~120 seconds)..."
    log_info "Java startup log: $BACKEND_DIR/logs/java.log"
fi
...
echo ""
echo "  Log files:"
echo "  - Agent:       $AGENT_DIR/logs/agent.log"
echo "  - Perception:  $PERCEPTION_DIR/logs/perception.log"
echo "  - Java:        $BACKEND_DIR/logs/java.log"
